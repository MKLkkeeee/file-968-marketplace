...
// ============ Lucky Wheel ============
export type WheelRewardType = "point" | "item" | "nothing";
export interface WheelSlice {
  id: string;
  label: string;
  type: WheelRewardType;
  /** points awarded when type === 'point' */
  pointValue?: number;
  /** multi-line stock for type === 'item'; one non-empty line = one deliverable */
  stockItems?: string;
  /** relative weight for randomness (default 1). Higher = more frequent. */
  weight: number;
  /** hex color for the slice background */
  color: string;
  active: boolean;
  createdAt: number;
}
export interface WheelConfig {
  enabled: boolean;
  spinCost: number; // points per spin
  updatedAt: number;
}
export interface WheelSpin {
  ...
  sliceLabel: string;
  type: WheelRewardType;
  reward: string; // delivered item or "+N point" or "-"
  cost: number;
  createdAt: number;
}

export const getWheelConfig = async (): Promise<WheelConfig> => {
  const snap = await get(ref(db, "wheel/config"));
  return snap.exists()
    ? (snap.val() as WheelConfig)
    : { enabled: false, spinCost: 50, updatedAt: 0 };
};
export const saveWheelConfig = (data: Omit<WheelConfig, "updatedAt">) =>
  set(ref(db, "wheel/config"), { ...data, updatedAt: Date.now() });

export const createWheelSlice = async (data: Omit<WheelSlice, "id" | "createdAt">) => {
  const r = push(ref(db, "wheel/slices"));
  await set(r, { ...data, id: r.key, createdAt: Date.now() });
};
export const updateWheelSlice = (id: string, data: Partial<WheelSlice>) =>
  update(ref(db, `wheel/slices/${id}`), data);
export const deleteWheelSlice = (id: string) => remove(ref(db, `wheel/slices/${id}`));

/** Atomically pop one stock line from a slice. Returns null if empty. */
export const popWheelStock = async (sliceId: string): Promise<string | null> => {
  let popped: string | null = null;
  await runTransaction(ref(db, `wheel/slices/${sliceId}/stockItems`), (current: string | null) => {
...
export const recordWheelSpin = async (data: Omit<WheelSpin, "id" | "createdAt">) => {
  const r = push(ref(db, "wheel/spins"));
  await set(r, { ...data, id: r.key, createdAt: Date.now() });
};
